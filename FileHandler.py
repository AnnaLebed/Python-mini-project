        ...
        try:
            ...
            for num, row in enumerate(loaded_data, start=0):
                if int(row[0]) == int(id):
                    ...
                    loaded_data.pop(num)
                    loaded_data.insert(num, data)
                    break

            # returning columns names back to the content
            loaded_data.insert(0, columns)
...
fh = FileHandler()
fh.update_csv('User.csv', 91111, [911111, 'updated first', 'updated last', 'password', 'manager', '80000', 'chef'])

# # fh.load_from_csv('User.csv')#
#
# print(fh.append_to_csv('User.csv', [1001000, 'Daniel', 'Leffiz', '101010', 'manager', 20000, 'employee']))
#
# print(fh.remove_from_csv('user.csv', 777777))
#
#
#
